...
subtitle: "Sensitivity, efficiency and reproducibility"
date: 2024-07-15
date-modified: today
draft: true
---

## Tests of hypotheses and model fitting

The guidelines given here for good experimental design have been
formulated with particular consideration given to UV manipulation
experiments. Nevertheless most of the principles behind these guidelines
can be generally applied to the design of experiments.

Experiments can be designed to test hypotheses or to estimate the values
of parameters in a model. In the first case, we should be careful not to
let our knowledge of the data influence the tested hypothesis[^35]. In
the second case, model choice may to some extent depend on the data, but
we should be aware that this *a posteriori* choice of a functional
relationship, could make the $P$-values less strict for testing
treatment effects.

Fitting models is especially useful when the parameters of the model
have a biological interpretation. In this case, it is important to
obtain estimates of the reliability of your estimates of the values
obtained for the parameters. In other words, we need to present not only
the fitted values, but also their confidence intervals, or at least
standard deviations.
...
Following D. R. @Cox1958 [Chapter 1]:

Precision.

:   Random errors of estimation should be suitably small, and this
    should be achieved with as few experimental units as possible.

Absence of systematic error.

:   Experimental units receiving different treatments should differ in
    no systematic way from one another.
...
:   A proper statistical analysis of the results should be possible
    without making artificial assumptions.
...
assumptions (Figure
[\[fig:principles:design:experiments\]](#fig:principles:design:experiments){reference-type="ref"
reference="fig:principles:design:experiments"}). In the sections below
we discuss each of these principles in turn and give examples of their
application.

::: figure*
1.  Replication $\rightarrow$ control of random variation $\rightarrow$
    precision.

2.  Randomization $\rightarrow$ elimination of systematic error
    $\rightarrow$ no bias.

3.  Use of blocks $\rightarrow$ reduction of error variation caused by
    experimental unit heterogeneity $\rightarrow$ increased power of
    statistical tests.
:::
...
::: inparaenum
It allows for estimation of the random variation accompanying the
results. This random variation is called *experimental error*.

Replication increases the precision with which the treatment effect is
estimated, because each repetition gives additional information about
...
::: inparaenum
The order in which measurements are done in the different experimental
...
If several treatments are applied to each experimental unit in a
sequential design, the order in which they are applied should be
randomized.

Everything that can be randomized, should be randomized to avoid bias,
...
It prevents systematic errors from known and unknown sources of
...
::: example*
[]{#ex:stats:block label="ex:stats:block"}
...
[\[fig:flowchart:exp:unit\]](#fig:flowchart:exp:unit){reference-type="ref"
reference="fig:flowchart:exp:unit"} summarizes the designs most suitable
for different situations based on the characteristics of the
experimental units.

:::: figure*
::: center
![image](figures/experimental_units_flow_chart.pdf)
:::
::::
...
into account the correlation among the repeated measurements. See Box
[\[ex:stats:units:samples\]](#ex:stats:units:samples){reference-type="ref"
reference="ex:stats:units:samples"} for examples, and the flowchart in
Figure
[\[fig:flowchart:meas:unit\]](#fig:flowchart:meas:unit){reference-type="ref"
reference="fig:flowchart:meas:unit"} for a summary of how the
...
:::: figure*
::: center
![image](figures/measurement_units_flow_chart.pdf)
:::
::::
...
The flowchart in Figure
[\[fig:flowchart:treatments\]](#fig:flowchart:treatments){reference-type="ref"
reference="fig:flowchart:treatments"} describes the relationships
between the structure of treatments and data analysis.

:::: figure*
::: center
![image](figures/treatment_assignment_flow_chart.pdf)
:::
::::
