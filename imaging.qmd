...
#| fig-cap: Spectral sensitivity of the three channels of the industrial colour camera Triton TRI071S-C from LUCID Vision Labs (Burnaby B.C. Canada) based on the RGB version of the Sony IMX482 image sensor. Data extracted from plot (EMVA 1288 Data) at https://thinklucid.com/ (visited on 2026-02-16).

autoplot(image_sensors.mspct[4], norm = "max",
         w.band = c(Plant_bands("Sellaro"), IR_bands()),
         range = c(250, 1100))
```
...
#| fig-asp: 2
#| fig-cap: Spectral sensitivity of three different monochromatic industrial cameras from LUCID Vision Labs (Burnaby B.C. Canada). Triton TRI071S-M  based on the monochrome version of the IMX482 CMOS image sensor from Sony; Atlas10 ATX081S-UC based on the special CMOS UV image sensor IMX487 from Sony; Triton2 TRT033S-WC based on the SWIR InGaAs image sensor IMX993 from Sony. Data extracted from plots (EMVA 1288 Data) at https://thinklucid.com/ (visited on 2026-02-16).

autoplot(image_sensors.mspct[c(1,3,2)], 
         facets = 1, norm = "max",
         w.band = c(Plant_bands("Sellaro"), IR_bands()),
         range = c(200, 1800))
```
...
#| tbl-cap:  Focal lengths corresponding to selected field of view angles for different sensor or film frame sizes (degrees). Diagonal stands for frame diagonal (mm) and factor stands for "sensor crop factor". Focal lengths are approximate. The TG-7 has a non-interchangeable lens covering angles from $80^\circ$ to $12^\circ$, and the iPhone 16, two lenses, convering angles of $80^\circ$ and $120^\circ$, respectively, with zooming done by cropping. 
...
Easier to read, more narrowly focused books, on UV photography [@Prutchi2017], UV and NIR photography [@Davies2017], photographying plants [@Blacklock1990;@Davies2023] and photographying the unseen [@Davies2020] are the best for readers with a minimum of familiarity with basic photography techniques. They complement well introductory books such as those listed above [@Banek2013;@Banek2013a].
