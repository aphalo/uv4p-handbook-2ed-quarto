...
A figure of spectral irradiance is difficult to read as both the shape of the curve and the maximum spectral irradiance change in parallel [@fig-spectra-sun-elevatio].
...
The spectral wavelength distribution is also changed by the amount of absorbing ozone in the atmosphere, known as the ozone column [@fig-UVB-PAR-ozone].
...
Above we have ignored the effects of clouds and relied heavily on simulations. Next we will consider the effect of clouds and use as examples measurements at different time scales.

Figure [] shows how spectral irradiance changes throughout one day. When the whole spectrum is plotted using a linear scale the effect of ozone depletion is not visible, however, if we plot only the region (Figure [\[fig:solarUV:spectrum:noon\]](#fig:solarUV:spectrum:noon){reference-type="ref" reference="fig:solarUV:spectrum:noon"}) the effect becomes clearly visible.
