...
In traditional multichannel boadband sensors, such as red and far-red sensors,
...
**A very frequent and grave mistake is to assume that a calibration provided by 
the sensor supplier, e.g., for sunlight is applicable to artificial light
sources, such as discharge lamps, incandescent lamps or LEDs.** Some suppliers
provide approximate correction factors (e.g. Apogee) or clear warnings in the
documentation, while others do not even warn users.
...
The most frequently measured irradiance in relation to plants, is the photon 
irradiance of photosynthetically active radiation ($Q_\mathrm{PAR}$) frequently
described as photosynthetic photon flux density and abbreviated PPFD. Given the
central role of photosynthesis, multiple radiation quantities have been proposed
over the years and a few competing ones remain currently in use to
quantify radiation that can drive the light reactions of photosynthesis.

The simplest approach is to define a range of active wavelengths and integrate
the energy irradiance or the photon irradiance within these limits. However,
conceptually, the use of photon or energy irradiance implies that either the
flux of photons or the flux of energy drives photosynthesis.
...
@GaastraXXXX and no longer in use in plant and agriculture research but still
used in meteorology, under the wrong name of PAR. **The relationship between
$Q_\mathrm{PAR}$ and $E_\mathrm{PhR}$ depends on the shape of spectrum of the
light measured.
...
However, the conversion is not unique, as it needs to normalised based on a 
...
erroneously assumed (even by this author in the past). PAR should always be
...
The definition by @McCree1972b was inspired by action spectra for photosynthesis
he had measured in leaves of several different crop plants. These action spectra
were based on absorbed photons rather than on the incident photon flux. When
we use PAR to quantify light we assume all photons in $400\ldots 700$ nm are
absorbed, which may be in many cases nearly true for vegetation canopies, but
almost never true for individual leaves.

PAR does not attempt to describe the light response of photosynthesis of plants
of any given species growing in a specific environment. PAR is a method for
quantification of light in a way relevant to plants in general, _approximating_
close enough the response expected from important crop plants. It is similar to
how the brightness of illumination is assessed based on a typical response of
human vision. Both standard definitions of coordinated colour temperature (CCT),
colour rendition index (CRI) and also PAR are designed to be most useful in
nearly white light.
...
was originally named Yield Photon Flux (originally YPF, here PQYR). As McCree
published two different mean action spectra, one for field-grown crop plants and
one for growth-chamber-grown crop plants we end up with two new variations of
PAR. This approach seems to also extend the wavelength range to that in the
measured action spectra. It must be kept in mind that the original spectra were
measured under single colour light in bands approximately xx nm wide spaced by
25 to 30 nm. Differently to with McCree's definition, when calculating these
quantities the weights ($a_q(\lambda)$) applied are extracted directky from the
...
the far red extending the wavelength range into $400\ldots 750$ nm. 

$$Q_\mathrm{ePAR} = \int_\mathrm{400\,nm}^\mathrm{750\,nm} Q(\lambda)\ d \lambda$$

However, the role of far red light in photosynthesis is synergistic, and known
as Emerson's effect. Far red light cannot drive photosynthesis on its own, but
only when shorter wavelengths are concurrently present with high enough photon
irradiance. Simple ePAR sensors like xxx from Apogee ignore this, and do give an
ePAR reading higher than zero under pure far-red radiation. When computing
extended PAR from spectral data, the VIS light requirement can be considered in
the calculations and the value capped when needed to account for the nature of
the response to far red. We use xPAR to describe the constrained version of 
ePAR.
...
spectral energy irradiance. The quantity proposed by used the same wavelength
range as PAR ($400\ldots 700$ nm) while that proposed by xxxx used a slightly
different one ($400\ldots 700$ nm).


With a spectroradiometer we can measure the spectral energy irradiance, if
...
described above. The table below shows the results of applying this approach
to several different natural and artificial light sources.
...
response close to that proportinal to the cosine of the angle of incidence of
...
simpler (usually cheaper) sensors deviate more from PAR's definition.
...
parallel measurement against a spectroradiometer. A photodiodes have a nearly
perfectly linear response to irradiance, in most case a single point calibration
is enough.
